...
//Envoi et stockage dans le localStorage
    let produitStorage = JSON.parse(localStorage.getItem("produit"));

//On demande à récupérer la balise colors lors du clic
...
//Si le tableau est nul, alors affiche moi les informations de d'Array de l'API
...
else if (produitStorage != null) {
//Rajouter un élément dans localStorage s'il y a déjà un 
    for ( i=0; i < produitStorage.lenght; i++);
        if(produitStorage[i]._id == canapProduit._id && 
            produitStorage[i].couleur == selectionCouleur.value) {
        return(
            produitStorage[i].quantite += Number(option_quantite.value),
            localStorage.setItem("produit", JSON.stringify(produitStorage))
            );  
        }
    ...
    for (i=0; i < produitStorage.lenght; i++);
        if (produitStorage[i]._id == canapProduit._id && 
        produitStorage[i].couleur != canapProduit.couleur);
        return(
            produitStorage.push(CHOIXCANAP),
            localStorage.setItem("produit", JSON.stringify(produitStorage))
            )
